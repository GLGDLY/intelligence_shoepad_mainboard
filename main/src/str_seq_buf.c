...
/* Private */
static inline void strbuf_init(StrSeqBuf_t* buf) { buf->sem = xSemaphoreCreateCounting(buf->size, 0); }
...
static void strbuf_write_nolock(StrSeqBuf_t* buf, const char* wr_buf, const size_t len) {
...
inline bool strbuf_is_empty(StrSeqBuf_t* buf) { return uxSemaphoreGetCount(buf->sem) == 0; }
inline uint32_t strbuf_get_items_count(StrSeqBuf_t* buf) { return uxSemaphoreGetCount(buf->sem); }

void strbuf_write(StrSeqBuf_t* buf, const char* wr_buf) {
	strbuf_check_init(buf);
	xSemaphoreGive(buf->sem);

	const size_t len = strlen(wr_buf) + 1; // include null terminator
...
void strbuf_read_all_with_action(StrSeqBuf_t* buf, StrSeqBufAction_t action, const size_t max_len) {
	...
	while (!strbuf_is_empty(buf)) {
		...
		strbuf_read(buf, tmp_buf, max_len);
		action(tmp_buf);
	}
}